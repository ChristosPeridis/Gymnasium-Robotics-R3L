...
class MujocoUR10eReachEnv(MujocoUR10eEnv, EzPickle):
    """
    ## Description
    ...
    The task in the environment is for a manipulator to move the end effector to a randomly selected position in the robot's workspace. The robot is a 7-DoF [UR10e Mobile Manipulator](https://UR10erobotics.com/) with a two-fingered parallel gripper.
    ...
    * `achieved_goal`: this key represents the current state of the end effector, as if it would have achieved a goal. This is useful for goal orientated learning algorithms such as those that use [Hindsight Experience Replay](https://arxiv.org/abs/1707.01495) (HER).
    ...
    To initialize this environment with one of the mentioned reward functions the type of reward must be specified in the id string when the environment is initialized. For `sparse` reward the id is the default of the environment, `UR10eReach-v2`. However, for `dense`
    reward the id must be modified to `UR10eReachDense-v2` and initialized as follows:

    ```python
    import gymnasium as gym

    env = gym.make('UR10eReachDense-v2')
    ```
    ...
    env = gym.make('UR10eReach-v2', max_episode_steps=100)
    ```
...
class MujocoPyUR10eReachEnv(MujocoPyUR10eEnv, EzPickle):
    def __init__(self, reward_type: str = "sparse", **kwargs):
        initial_qpos = {
